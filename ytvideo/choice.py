...
def generate_youtube_script(topic):
    prompt = f"""
    write youtube script for {topic} in narrating style without intro and outro, be concise and precise straight to the point only content of topic nothing else, dont use bold text to emphasize or any mark like **, i just want plain normal format , it should have just script and nothing else mind it, video should be engaging and valuable to viewers, focus on quality and engagement,  i want video to be viral and generate infinite money with it so make script that good. script must be in paragraph format(required dont forget it mind it) dont write in bold anything script should be in paragraph format against youtube script word plainly and nothing else in it as i have to fed it to my ai program so making it understand would be easy so just assign script plainly against youtube script word,  script length msut be of max 4 minute must mind it, script msut be creative and engaging mind it i need it okay.
    """
...
if __name__ == "__main__":
    ...
    while True:
        ...
        # print("\n📜 Generated Script:\n", script)
        # output_path= create_video_from_script(script, user_topic)
        ...
        if choice == "1":
            ...
            if user_topic.lower() not in existing_topics:
                with open(TOPIC_LOG_FILE, 'a', encoding='utf-8') as file:
                    file.write(user_topic + '\n')
            script = generate_youtube_script(user_topic)
            print("\n📜 Generated Script:\n", script)
            ouput, used_urls=create_video_from_script(script, user_topic)
        ...
        output_path= "video_created/output_with_audio.mp4"
