...
def create_video_from_script(script, user_topic, output_video_path):
    from nltk.tokenize import sent_tokenize
    ...
    # Background music
    bg_music = AudioFileClip("/Users/uday/Downloads/VIDEOYT/Cybernetic Dreams.mp3") \
                    .set_start(5).set_duration(final_clip.duration).volumex(0.03)

    # Mix with existing audio (tick sounds, etc.)
    if final_clip.audio:
        final_audio = CompositeAudioClip([final_clip.audio.set_duration(final_clip.duration), bg_music])
    else:
...
def generate_videos_from_folder(course_folder,topic):
    ...
    for filename in sorted(os.listdir(course_folder)):
        if filename.endswith(".txt") and filename != "course_outline.txt":
            ...
            create_video_from_script(script, topic, output_video_path)
