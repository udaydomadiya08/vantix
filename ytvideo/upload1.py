...
if __name__ == "__main__":
    ...
    # === CONFIGURATION ===
    video_folder_path = '/Users/uday/Downloads/VIDEOYT/final_video'  # Replace with your actual folder
    text_file_path = '/Users/uday/Downloads/VIDEOYT/used_topics.txt'    # Replace with your actual file path
    video_extensions = ['.mp4', '.avi', '.mov', '.mkv', '.flv', '.wmv']

    # === STEP 1: Get newest-to-oldest video files ===
    video_files = sorted(
        [
    ...
    # === STEP 2: Get text lines from last to first ===
    with open(text_file_path, 'r', encoding='utf-8') as file:
        text_lines = file.readlines()[::-1]  # reverse to go last-to-first

    # === STEP 3: Manual loop with index to break if any list is shorter ===
    max_len = min(len(video_files), len(text_lines))  # Safeguard for shortest length

    for i in range(max_len):
        current_video = video_files[i]
        current_text = text_lines[i].strip()

        print("Video:", current_video)
        print("Text: ", current_text)

        success= upload_video(current_video, current_text, "/Users/uday/Downloads/VIDEOYT/final_thumbnail.jpg")
        if success:
            # Delete the local video file only if upload succeeded
            try:
                os.remove(current_video)
                print(f"Deleted local video: {current_video}")
            except Exception as e:
                print(f"Error deleting file {current_video}: {e}")

    # Optional message
    if len(video_files) != len(text_lines):
        print("Stopped early — one list ran out of items.")
