...
def generate_gemini_response(prompt, model_name=None, max_retries=11, wait_seconds=5):
    import google.generativeai as genai
...
# ✅ Compress image to stay under 2MB
def compress_thumbnail(input_image: Image.Image, output_path: str, max_size_kb=2048):
    quality = 90
    while quality >= 20:
...
def generate_thumbnail_with_multiple_keys(topic: str = "{topic}"):
    prompt = (
        ...
        "Thumbnail must be vertical, mobile-fit in vertical ratio 9:16, mind it."
    )
        ...
        if part.text:
            ...
            if image_data:
                try:
                    ...
                    output_path = "resized_thumbnail_1080x1920.jpg"
...
def generate_youtube_description(topic):
    ...
    It should be keyword-rich and increase viewer interest. mind it description should bes seo optimised
...
def generate_youtube_tags(topic):
    ...
    Only return the tags in a list format separated by commas. Do not include extra explanation or formatting. it should be seo optimised mind it, just return list of tags comma separted thats it,  mind it it should bes seo optimised
    """
    response = generate_gemini_response(prompt, model_name="gemini-2.5-flash-preview-05-20")
...
def upload_video(file_path, topic):
    ...
    topic=generate_title_with_hashtags(topic)
    # Ensure #shorts in title and description
    if "#shorts" not in topic.lower():
        topic_with_shorts = topic + " #shorts"
    else:
        topic_with_shorts = topic

    description = generate_youtube_description(topic_with_shorts)
    description = generate_description_with_scene_links(description, feedback_link, file_path)

    # Append #shorts in description if not present
    ...
    # Ensure essential shorts-related tags
    shorts_tags = {"shorts", "youtube shorts", "viral shorts"}
    # Add shorts tags only if not already present (case insensitive)
    ...
    # Category ID resolution (optional: you can force People & Blogs = 22)
    ...
    logger.info(f"🎬 Title: {topic_with_shorts}")
