...
def generate_youtube_script():
    prompt = f"""
    "Write a powerful and emotionally uplifting motivational video script, using a cinematic tone, personal storytelling, and practical life advice. Structure it with a strong hook, emotional buildup, and a climactic ending that inspires massive action. Keep it under 2 minutes, with short impactful sentences that can be overlaid with video clips and music. Include a famous quote. only give plain fromat script dont use bold text plain fromat normal text and script only nothing else other than it mind it i wan tonly plain for mat normal script and nothing elsae"
    """
...
def get_topic_from_script(script):
    ...
    should be seo optimise catch and attractive tirl that user tend to click video
    Script:
    \"\"\"
    {script}
    \"\"\"

    Only return the title as plain text without quotes or extra text. ntohing else mind it
    """
