...
def generate_youtube_script():
    prompt = f"""
    “Write a short, highly emotional motivational story that inspires confidence, discipline, and a never-give-up attitude. It should be 3–5 paragraphs long, include a relatable main character facing a major challenge, and end with a powerful lesson or quote. Use vivid language, strong emotions, and cinematic scenes to engage the viewer. Keep it short enough for a 2 minute voiceover script. Avoid complex vocabulary—make it suitable for all audiences. only give plain fromat script dont use bold text plain fromat normal text and script only nothing else other than it mind it i wan tonly plain for mat normal script and nothing elsae"
    """
...
@retry_infinite(delay=5)    
def get_topic_from_script():
    prompt = f"""
    Generate a single, clear **topic** and a detailed **script** for a YouTube video that can become very popular and potentially generate high income.

    - First, choose an interesting, engaging topic by yourself (like “Top 10 facts about space” or “Amazing life hacks for productivity”).
    - Then, write a well-structured script of about 300-500 words related to that topic.

    Please output ONLY in the following JSON format (without extra text):

    {
    "topic": "<your chosen topic here>",
    "script": "<the full video narration script here>"
    }
...
@retry_infinite(delay=5)
def search_pixabay_videos(query, per_page=11, max_results=1):  # lowered per_page for quicker tests
...
import json
@retry_infinite(delay=5)
def search_pexels_video(query, per_page=11, target_width=1920, target_height=1080, tolerance=200, max_clips=1):
    page = 1
    headers = {"Authorization": PEXELS_API_KEY}
    suitable_clips = []
...
@retry_infinite(delay=5)
def find_one_video_clips(sentence, user_topic, max_clips=1):
...
@retry_infinite(delay=5)
def create_scene(text, idx, user_topic):
    print(f"\n🎬 Creating Scene {idx + 1}")
    
    video_clips_data = find_one_video_clips(text, user_topic, max_clips=1)
...
def create_fast_cut_clip_from_images(image_paths, total_duration=5, resolution=(1920,1080)):
    ...
    tick_sound = AudioFileClip("/Users/uday/Downloads/VIDEOYT/analog-camera-shutter-96604_z7Dhy2kD.mp3").volumex(0.5)
    clips = []
...
def create_video_from_script(script, user_topic):
    ...
    for idx, sentence in enumerate(sentences):
        scene_clip= create_scene(sentence, idx, user_topic)
    ...
    # Background music
    bg_music_raw = AudioFileClip("/Users/uday/Downloads/VIDEOYT/Cybernetic Dreams.mp3").volumex(0.1)
    ...
    # ✅ Write final video WITH audio using MoviePy directly (no FFmpeg needed)
...
if __name__ == "__main__":
    ...
    while True:
        ...
        try:
            ...
            print("❌ Gemini didn't return valid JSON. Full response:\n", result)
        video_path= create_video_from_script(script, user_topic)
