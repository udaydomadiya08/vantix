...
def generate_youtube_shorts_script(topic):
    prompt = f"""
    write a youtube shorts script for {topic} in a fast-paced, high-impact narrating style without any intro or outro. the script must be plain text only with no bold text, no bullet points, no formatting marks — just one single paragraph of pure content. make it concise, valuable, and highly engaging, crafted to instantly hook viewers and maintain interest throughout. it must be optimized for viral performance and infinite monetization potential. total duration must not exceed 1 minute by gtts speaker as i am using that, so keep the script within 10 to 14 short sentences max. do not add any headings or labels, only the script paragraph. this will be fed directly into an ai video generation system so formatting must be clean and plain. keep energy high, delivery fast, and message clear. no fluff, just pure value.
    """
...
def generate_audio(text, output_file):
    ...
    # Step 1: Generate TTS using gTTS
    tts = gTTS(text=text, lang='en')
    tts.save(temp_file)

    # Step 2: Speed up audio with ffmpeg
    subprocess.run([
    ...
    # ✅ Return just the path — MoviePy will use it
    return output_file
...
def create_scene(text, idx, used_video_urls, user_topic):
    ...
    try:
        ...
        audio_path = generate_audio(text, audio_path)
    ...
    subtitle = (
        ...
        .set_position(("center", 1620))  # Near bottom
        .set_duration(audio_duration)
        .fadein(1)
    )
...
if __name__ == "__main__":
    ...
    def generate_viral_wealth_topic():
        ...
        prompt = [
            ...
            From  subcategories listed above, chose randomly one give me:
    ...
    print("\n📜 Generated Script:\n", script)
    output_file, used_urls= create_video_from_script(script, user_topic)
