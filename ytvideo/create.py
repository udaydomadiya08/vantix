...
def generate_youtube_script(topic):
    ...
    The final script must not exceed what can be spoken in less than  4 minutes using gTTS (around 350-400 words, or ~25-30 concise sentences). This will be used by an AI video generator, so keep formatting clean and the paragraph tight. Prioritize viral potential, monetization value, and viewer retention. at end tell user to like comment and subscribe to my channel tell it dont ask it if they liked then do just tell them to do all 3 at end, okay mind it,  mind it it should bes seo optimised
    dont inlude astreiks bold text only plain format normal text mind it, be concise and precise straight to the point only content of topic nothing else, , dont use bold text to emphasize or any mark like **, i just want plain normal format , it should have just script and nothing else mind it, video should be engaging and valuable to viewers, focus on quality and engagement,  i want video to be viral and generate infinite money with it so make script that good. script must be in paragraph format(required dont forget it mind it) dont write in bold anything script should be in paragraph format, plainly and nothing else in it as i have to fed it to my ai program so making it understand would be easy so just assign script plainly,  script length msut be of  max 21 sentences only or max 3 minutes  and not more than this criteria mind it, script msut be creative and engaging mind it i need it okay.
    """
...
import json
@retry_infinite(delay=5)
def search_pexels_video(query, per_page=80, target_width=1920, target_height=1080, tolerance=200, max_clips=1):
    page = 1
    headers = {"Authorization": PEXELS_API_KEY}
    suitable_clips = []
...
def generate_tts_audio(text, filename="output.mp3", json_key_path="my-project-tts-461911-dbd39de52028.json",
                       voice_name="en-US-Studio-O", speaking_rate=1.2):
...
def create_scene(text, idx, used_video_urls, user_topic):
    ...
    def resize_crop(clip):
        ...
        if abs(actual_aspect - target_aspect) < 0.01:
            ...
            x_center = clip.w / 2
            return clip.crop(x1=x_center - 960, x2=x_center + 960, y1=0, y2=1080)
        else:
            # Taller → crop height
            clip = clip.resize(width=1920)
            y_center = clip.h / 2
            return clip.crop(x1=0, x2=1920, y1=y_center - 540, y2=y_center + 540)
    ...
    try:
        ...
        subtitle = create_word_by_word_subtitles(word_segments, video_size=(1920, 1080))
    except Exception as e:
...
def create_video_from_script(script, user_topic):
    ...
    scene_clips = [clip.resize((1920, 1080)).set_fps(30) for clip in scene_clips]
...
if __name__ == "__main__":
    ...
    while count:
        ...
        timestamp = datetime.now().strftime("%Y%m%d_%H%M%S")
        output_file = f"/Users/uday/Downloads/VIDEOYT/final_video/video_{timestamp}.mp4"
        ...
        command = [
            ...
            "-filter_complex",
            """
            [1:v]scale=444:667,setpts=PTS+10/TB[avatar];
            [0:v][avatar]overlay=W-w-10:H-h-10:enable='gte(t,10)'[outv];
